...
class Robinhood(InstrumentManager, SessionManager):
    ...
    def get_options(self, stock, expiration_dates, option_type):
        ...
        results = self.get_url(urls.build_chain(instrument_id))["results"]
        try:
            if len(results) > 1:
                for r in results:
                    if r['symbol'] == stock and r['can_open_position'] == True:
                        chain_id = r["id"]
                        break
            else:
                chain_id = results[0]["id"]
        except:
            chain_id = results[0]["id"]
        return [
            contract
    ...
    def option_order_history(self, orderId=None):
        ...
        return self.get('https://api.robinhood.com/options/orders/{}/'.format(orderId))
    ...
    def place_option_market_buy_order(
        self, instrument_URL=None, symbol=None, time_in_force=None, quantity=None,ask=None, bid=None,ourpirce=None
    ):
        """Wrapper for placing market buy orders
    ...
    def place_option_market_sell_order(
            self, instrument_URL=None, symbol=None, time_in_force=None, quantity=None, ask=None, bid=None, ourpirce=None
    ):
        """Wrapper for placing market sell orders
        ...
        return self.submit_sell_order(
            ...
            symbol=symbol,
            time_in_force=time_in_force,
            quantity=quantity,ask=ask, bid=bid, ourpirce=ourpirce
        )
    ...
    def submit_sell_order(  # noqa: C901
        ...
        quantity=None,
        side=None,
        ask=None, bid=None, ourpirce=None
    ):
        ...
        if ask != None:
            payload= {
                ...
                "client_ask_at_submission": ask,
                "client_bid_at_submission":bid,
                "direction": "credit",
                "form_source": "strategy_detail",
                "legs": [
                    {
                        "option": instrument_URL,
        ...
        return res
    def submit_option_sell_order(  # noqa: C901
        self,
        instrument_URL=None,
        symbol=None,
        order_type=None,
    ...
    def cancel_option_order(self, order_id):  # noqa: C901
        """Cancels specified order and returns the response.
        ...
        if isinstance(order_id, str):
            try:
                order = self.get('https://api.robinhood.com/options/orders/{}/'.format(order_id))
            except (requests.exceptions.HTTPError) as err_msg:
                raise ValueError(
                    "Failed to get Order for ID: "
                    + order_id
            ...
            if order.get('cancel_url') is not None:
                try:
                    res = self.post(order['cancel_url'])
                    return res
                except requests.exceptions.HTTPError:
                    try:
                        # sometimes Robinhood asks for another log in when placing an
